...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	peer chaincode install -n sacc -v 1.0 -p github.com/hyperledger/fabric/examples/sacc >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n sacc -v 1.0 -c '{"Args":[ ]}'  >&log.txt
	else
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n sacc -v 1.0 -c '{"Args":[]}' >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeInvoke () {
	...
	# lets supply it directly as we know it using the "-o" option
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
	    peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n sacc -c '{"Args":["iPostUsers", "1001", "USER", "email", "password", "firstname", "lastname", "91001"]}' > log.txt
	else
	    peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n sacc -c '{"Args":["iPostUsers", "1001", "USER", "email", "password", "firstname", "lastname", "91001"]}' > log.txt
	fi
	res=$?
